...
import SollutionDialog from "./SollutionDialog";
...
function PastContests({ filters, search }: { filters: string[], search: string }) {
  ...
  useEffect(() => {
    ...
    let url = `/contests?&endDate=${new Date().toUTCString()}&page=${page}&search=${search}`;
    if (platformQuery) {
    ...
    api
      ...
      .then((res) => {
        if (res.data.ok) {
          ...
          if (user) {
            const bookmarkedContests = user.bookmarks.map(
              (bookmark: Contest) => bookmark._id
            );
            ...
            >
              ...
              <div className="flex flex-col items-end justify-between">
                ...
                {contest.solution_link ? (
                  ...
                  user?.role === "admin" && (
                    <SollutionDialog contestId={contest._id} />
                  )
                )}
              </div>
            </li>
